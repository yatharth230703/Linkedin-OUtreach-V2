...
def get_leads_from_file(filename="leads.json"):
    try:
        # Look in parent directory (Linkedin_cloud_bot)
        ...
        print("   leads.json not found. Using dummy data.")
        return []
...
def remove_lead_from_file(url, filename="leads.json"):
    ...
    try:
        ...
        if url in leads:
            ...
            print(f"      Removed {url} from leads.json")
            return True
    except Exception as e:
        print(f"      Could not remove lead from file: {e}")
        return False
    ...
    try:
        ...
        if "profile_posts" in str(e) or "message_2_draft" in str(e) or "message_3_draft" in str(e):
            try:
                ...
                print(f"   Note: profile_posts and follow-up message columns not available")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='LinkedIn Connection Bot with Template Support (Playwright)')
    ...
    try:
        ...
        if not target_urls:
            print("   No leads found in leads.json. Exiting.")
            return
        ...
        for url in target_urls:
            ...
            if exists:
                ...
                print(f"   Skipping: Lead already in DB (Status: {status})")
                continue
