...
env | grep -E '^(ATTIO_API|ATTIO_API_ALT|GEMINI_API_KEY|APIFY_API|PROXY_HOST|PROXY_PORT|PROXY_USERNAME|PROXY_PASSWORD_BASE|PROXY_PASSWORD|USE_PROXY|SKIP_COOKIE_LOGIN|DISABLE_STEALTH|CLOUD_MODE|DISPLAY|SLACK_WEBHOOK_URL|SLACK_WEBHOOK_URL_ALT|YATH_LINKEDIN_EMAIL|YATH_LINKEDIN_PASSWORD|MAURICE_LINKEDIN_EMAIL|MAURICE_LINKEDIN_PASSWORD|LEON_LINKEDIN_EMAIL|LEON_LINKEDIN_PASSWORD)=' | while read -r line; do
    key="${line%%=*}"
    val="${line#*=}"
    printf 'export %s=%q\n' "$key" "$val" >> "$ENV_FILE"
done
