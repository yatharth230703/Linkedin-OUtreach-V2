"""
LinkedIn Reply Detection Script
================================
Checks if connections we've messaged have replied to us.
If a reply is detected, prints "replied already" and updates Supabase status to "REPLIED".
"""
...
def main():
    """Main function to check for replies from LinkedIn connections"""

    print("=" * 60)
    print("🔍 LinkedIn Reply Detection Script")
    print("=" * 60)
    ...
    print("✅ LinkedIn session established. Starting reply check...")
    ...
    try:
        ...
        # Scrape connections and identify leads to check
        leads_to_check = scrape_connections_for_reply_check(driver)

        if leads_to_check:
            print(f"\n🎯 Found {len(leads_to_check)} leads to check for replies!")

            # Start reply checking process
            replied, no_reply, errors = check_all_leads_for_replies(driver, leads_to_check)

            print(f"\n🏁 Reply check completed!")
            print(f"   ✅ Found {replied} replies")
            print(f"   ❌ No reply from {no_reply} leads")
        else:
            print("\n📭 No leads to check for replies.")
            print("   Either no leads have been messaged, or none are visible on connections page.")
